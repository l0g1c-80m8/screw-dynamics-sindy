...
class ModelTrainer():
    ...
    def __init__(self, training_params, sindy_params):
        ...
        ###Defining losses
        self.pose_reconstruction = torch.nn.MSELoss()
    ...
    def train(self):
        ...
        X_dev, x_dot_dev = next(iter(dev_data_loader))
        
        X_dev = X_dev.to(torch.device(self.device))
        x_dot_dev = x_dot_dev.to(torch.device(self.device))

        pose_dev = X_dev[:,:,2:8]
        pose_dev = pose_dev.to(self.device)
        
        wrench_dev = X_dev[:,:,8:14]
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_id, (X,x_dot) in enumerate(train_data_loader):
                ...
                [predicted_x_dot, pose_decoded, wrench_decoded] = self.model(X)
                end_time = time.time()
                print("Model Inference Time: ", end_time-start_time)
                pose = X[:,:,2:8]
                pose = pose.to(torch.device(self.device))
                wrench= X[:,:,8:14]
                ...
                pose_train_reconstruction = self.pose_reconstruction(pose,pose_decoded)
                current_epoch_pose_train_loss += pose_train_reconstruction.item()
                ...
                train_loss = sindy_train_loss + pose_train_reconstruction + wrench_train_reconstruction
            ...
            current_epoch_pose_train_loss = current_epoch_pose_train_loss/len(train_data_loader.dataset)
            ...
            current_epoch_pose_dev_loss = pose_dev_reconstruction.item()

            print("Development Losses at epoch: ", epoch + 1, " are as follows: ")
            print("1. Sindy Loss: ", current_epoch_sindy_dev_loss)
            print("2. Pose Reconstruction Loss: ", current_epoch_pose_dev_loss)
            ...
            if(((epoch+1) % self.log_freq) == 0):
                ...
                if(self.wandb):
                    wandb.log({'train_loss':current_epoch_train_loss,'train_sindy_loss':current_epoch_sindy_train_loss, 'train_pose_reconstruction': current_epoch_pose_train_loss,\
                            'train_wrench_reconstruction': current_epoch_wrench_train_loss,\
                            'dev_loss':current_epoch_dev_loss,'dev_sindy_loss':current_epoch_sindy_dev_loss, 'dev_pose_reconstruction': current_epoch_pose_dev_loss,\
    ...
    def evaluate(self):
        ...
        X_test = np.reshape(X_test,(self.time_horizon,14))
        ...
        plt.figure(1)
        plt.subplot(221)
        plt.plot(t,tests_predicted_x_dot[:,0],'b')

        plt.subplot(222)
        plt.plot(t,x_dot_test[:,0],'g')

        plt.subplot(223)
        plt.plot(t,X_test[:,0],'r')

        plt.figure(2)
        plt.subplot(221)
        plt.plot(t,tests_predicted_x_dot[:,1],'b')

        plt.subplot(222)
        plt.plot(t,x_dot_test[:,1],'g')

        plt.subplot(223)
        plt.plot(t,X_test[:,1],'r')
...
def main():
    ...
    parser.add_argument('-p', '--pose_dim', default=64, type=int, help='Latent space dim for pose')
    ...
    parser.add_argument('-', '--sindy_loss_weight', default=1e-4, type=float, help='Sindy Loss Scaling Factor')
    ...
    sindy_params['latent_dim'] = training_params_dict['state_space_dim'] + training_params_dict['pose_dim'] + training_params_dict['wrench_dim']
    ...
    sindy_params["pose_dim"] = training_params_dict["pose_dim"]
